...
def benchmark_pinocchio(model, data, q_list, qd_list, qdd_list, frame_id, 
                       reference_frame, num_repeats, warmup_iters):
...
def verify_correctness(accel, q, qd, qdd, bard_frame_id, pin_frame_id, reference_frame, 
    ...
    a_bard = accel.calc(q[:1], qd[:1], qdd[:1], bard_frame_id, 
                       reference_frame=reference_frame)[0].cpu().numpy()
...
def main():
    ...
    print(f"Acceleration size: (6,) - [linear_x, linear_y, linear_z, angular_x, angular_y, angular_z]")
    ...
    for reference_frame in ["world", "local"]:
        ...
        for batch_size in BATCH_SIZES:
            ...
            if batch_size == BATCH_SIZES[0] and reference_frame == "world":
                ...
                if not verify_correctness(accel, q, qd, qdd, bard_frame_id, pin_frame_id, 
                                         reference_frame, pin_model, pin_data, 
            ...
            # Benchmark Bard
            print("\nBenchmarking Bard...")
            bard_times = benchmark_bard(accel, q, qd, qdd, bard_frame_id, reference_frame,
