...
def bench_bard(model, data, q, nrep, nwarm):
    for _ in range(nwarm):
        bard.update_kinematics(model, data, q)
        _ = bard.crba(model, data)
    if DEVICE == "cuda":
...
def verify_short(model, data, q, pin_model, pin_data, q_pin):
    bard.update_kinematics(model, data, q[:1])
    M_bard = bard.crba(model, data)[0].detach().cpu().numpy()
