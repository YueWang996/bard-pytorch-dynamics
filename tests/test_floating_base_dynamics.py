...
class TestDynamicsFloatingBase:
    ...
    def test_rnea_full(self, urdf_string, pin_model, dtype, device):
        ...
        q_pin = np.concatenate([q_bard[:3].cpu().numpy(), q_bard[4:7].cpu().numpy(), q_bard[3:4].cpu().numpy(), q_bard[7:].cpu().numpy()])
    ...
    def test_crba_and_consistency(self, urdf_string, pin_model, dtype, device):
        ...
        tau_rnea_inertia = calc_inverse_dynamics(bard_chain, q_bard, torch.zeros_like(qdd_bard), qdd_bard, gravity=torch.zeros(3))
