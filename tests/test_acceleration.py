...
def compare_accelerations(a_bard, a_pin, dtype):
    ...
    if max_diff >= fail_tol:
        # Hard fail
        assert False, (f"Acceleration mismatch: max_diff={max_diff:.3e} > fail_tol={fail_tol:.1e}\n"
                      f"Linear diff: {linear_diff:.3e}, Angular diff: {angular_diff:.3e}"
        )
    elif max_diff >= warn_tol:
        # Warning but pass
        warnings.warn(f"Acceleration mismatch: max_diff={max_diff:.3e} > fail_tol={warn_tol:.1e}\n"
                      f"Linear diff: {linear_diff:.3e}, Angular diff: {angular_diff:.3e}"
        )
...
class TestSpatialAcceleration:
    ...
    def test_fixed_base_random_states(self, urdf_string, pin_model_fixed, dtype, device, reference_frame):
    ...
    def test_fixed_base_batched(self, urdf_string, pin_model_fixed, dtype, device):
        ...
        a_bard_batch = accel.calc(q_batch, qd_batch, qdd_batch, bard_frame_idx, 
                                   reference_frame="world").cpu().numpy()
        ...
        for i in range(batch_size):
            ...
            pin.forwardKinematics(pin_model_obj, pin_data, q_pin, qd_pin, qdd_pin)
            a_pin = pin.getFrameAcceleration(pin_model_obj, pin_data, pin_frame_id, 
                                            pin.ReferenceFrame.WORLD).vector
            
            compare_accelerations(a_bard_batch[i], a_pin, dtype)

    def test_fixed_base_zero_velocity_acceleration(self, urdf_string, pin_model_fixed, dtype, device):
        ...
        pin.forwardKinematics(pin_model_obj, pin_data, q[0].cpu().numpy(), 
                             qd[0].cpu().numpy(), qdd[0].cpu().numpy())
        a_pin = pin.getFrameAcceleration(pin_model_obj, pin_data, pin_frame_id, 
                                        pin.ReferenceFrame.WORLD).vector
    ...
    @pytest.mark.parametrize("reference_frame", ["world", "local"])
    def test_floating_base_random_states(self, urdf_string, pin_model_floating, dtype, device, reference_frame):
    ...
    def test_floating_base_stationary(self, urdf_string, pin_model_floating, dtype, device):
        ...
        pin.forwardKinematics(pin_model_obj, pin_data, q_pin, qd[0].cpu().numpy(), qdd[0].cpu().numpy())
        a_pin = pin.getFrameAcceleration(pin_model_obj, pin_data, pin_frame_id, 
                                        pin.ReferenceFrame.WORLD).vector
